...
public class ClienteRest implements Serializable {
	...
	public Pagamento criarPagamento(Transacao transacao) throws URISyntaxException {
	...
	public Pagamento confirmarPagamento(Pagamento pagamento) throws URISyntaxException {
		Link linkConfirmarPagamento = pagamento.getLinkPeloRel("confirmar");
		Client cliente = ClientBuilder.newClient();
		Pagamento resposta = cliente.target(SERVER_URI + linkConfirmarPagamento.getUri())
				.request()
				.build(linkConfirmarPagamento.getMethod())
				.invoke(Pagamento.class);
